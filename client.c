...
int main(int argc, const char** argv){
	...
	strcpy(fileName, "certificates/CACertificate.pem");
	file = fopen(fileName, "r");
	if(file == NULL){
		perror("Error during opening of the file CACertificate.pem\n");
		exit(-1);
	}
	...
	ret = EVP_SignUpdate(signCtx, opBuffer, dimOpBuffer);
	if(ret == 0){
